...
- Keep the contact-normalisation work small and isolated: only consolidate phone/text values and fix mismatched `tel:`/`sms:` links.
...
- Public text CTA behavior using the same approved phone number.
...
- Settings form: single-record editor for phone/text contact values, hero, CTA, and SEO defaults.
...
- Validation: require safe CTA paths or approved external URLs; validate phone URI formatting.
